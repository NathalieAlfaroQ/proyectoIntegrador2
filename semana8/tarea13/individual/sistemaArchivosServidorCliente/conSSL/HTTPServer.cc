...
int main() {
   ...
   // Socket* servidor = new Socket('s');
      SSLSocket* servidor = new SSLSocket( (const char *) "./ci0123.pem", (const char *) "./key0123.pem", false );

    servidor->Bind(PORT);        // Asocia el socket al puerto especificado
    servidor->MarkPassive(10);    // Escucha conexiones entrantes (máx. 5 en espera)
