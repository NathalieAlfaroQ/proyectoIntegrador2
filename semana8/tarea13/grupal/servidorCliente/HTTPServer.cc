...
std::string readHttp(VSocket& sock) {
    std::string data;
    char c;
    ...
    while (true) {
        ...
        if (n <= 0) {
            throw std::runtime_error("Conexión cerrada antes de completar headers");
        }
        data.push_back(c);
    ...
    size_t contentLength = 0;
    {
        ...
        if (clPos != std::string::npos) {
            ...
            std::string value = data.substr(clPos + 15, endPos - (clPos + 15));
...
void task(VSocket* client) {
 ///   FileSystem fs();     // Instancia para manejar figuras en el sistema de archivos
    //char buffer[BUFSIZE];
    //memset(buffer, 0, BUFSIZE);          // Inicializa el buffer con ceros

    try {
           // Lee la solicitud del cliente
        std::string request = readHttp(*client);
        ...
        if (path == "/figura") {
            ...
            if (pos != std::string::npos) {
                ...
                std::string figura = fs->get(nombre);
        ...
        // Manejo de solicitud para agregar una figura al sistema
            ...
            if (pos != std::string::npos) {
                ...
                std::string contenido = request.substr(request.find("\r\n\r\n"));

                // Intenta agregar la figura desde carpeta Figuras/
                if (!fs->add(nombre, contenido)) {
