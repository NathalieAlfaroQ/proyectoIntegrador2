...
{
   // Para guardar el mensaje en un buffer de tipo char inicializado en 0
   char a[BUFSIZE] = {0};

   // Leer el mensaje del cliente
   client->Read(a, BUFSIZE);
   std::string nombreFigura(a);

   // Muestra el mensaje del servidor
   cout << "Servidor recibe: " << nombreFigura << endl;
   std::string figura = leer_figura(nombreFigura);
   client->Write(figura.c_str());
   client->Close();
}
