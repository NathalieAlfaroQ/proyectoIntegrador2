...
int VSocket::EstablishConnection( const char * hostip, int port ) {
  int st = -1;
    ...
    if ( -1 == st ) {
      ...
      throw std::runtime_error( std::string("VSocket::EstablishConnection: connect failed. Errno: ")
                  + std::to_string(errno) + " - " + strerror(errno)
                  + ", address: " + addr_info + " - port: " + std::to_string(port));
    }
  }
...
int VSocket::WaitForConnection( void ) {
  ...
  if (st < 0) {
    ...
    throw std::runtime_error( std::string("VSocket::WaitForConnection: accept(fd=") + std::to_string(this->idSocket) + ") failed. Errno: " + std::to_string(err) + " - " + std::string(strerror(err)) );
  }
...
size_t VSocket::returnFd() {
  return idSocket;
}

size_t VSocket::returnPort() {
  return port;
}