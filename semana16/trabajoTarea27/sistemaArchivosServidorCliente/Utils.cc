...
std::string getLocalIP() {
    Socket s('d');
    s.MakeConnection("8.8.8.8", 80);

    struct sockaddr_in name {};
    memset(&name, 0, sizeof(name));
    socklen_t namelen = sizeof(name);

    if (getsockname(s.returnFd(), (struct sockaddr*)&name, &namelen) < 0) {
        return "0.0.0.0";
    }
...
bool waitMessage(Socket& s, int timeoutSec, std::string& outMsg, sockaddr_in& outSender) {
    ...
    inet_aton(getLocalIP().c_str(), &self.sin_addr);

    std::cout << "[waitMessage] Ignorando mensajes de IP=" << getLocalIP()
              << " PORT=" << s.returnPort() << "\n";
